...
import (
	...
	dceplugin "github.com/paypal/dce-go/plugin"
...
func init() {
	...
	dceplugin.Monitors.Register(&monitor{}, name)
...
func (m *monitor) Start(ctx context.Context) (types.PodStatus, error) {
	...
	run := func() (types.PodStatus, error) {
		for i := 0; i < len(pod.MonitorContainerList); i++ {
			...
			if err != nil {
				logger.Error("POD_MONITOR_FAILED -- Send Failed")
			...
			if exitCode != 0 {
				logger.Errorf("POD_MONITOR_APP_EXIT -- Stop pod monitor")
